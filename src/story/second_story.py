def second_story():
    ...
    if choice_search == 3:
        type_text('''
    Ты прислушиваешься к звукам, в полной тишине пытаешься услышать хоть что-то. Начинаешь с настороженностью и легкой опаской
    всматриваться в глубь леса. Брось кубик D20 для проверки внимания:
    ''')
        
        a = int(input('Для броска кубика напиши "20". \nВведи значение: '))
    ...
    Враг медленно поднимается в воздух, тяжело рассмотреть черты лица в этом силуэте. Вместо глаз и рта видна лишь непроглядная тьма.
            '''
    ...
    Полурослик-плут(наклонясь к войну): «{pers[1]["name"]} попробуй ты вглядеться, может существо, что изворотило куче еще тут?»''')
        ...
        {can_see}:
            ''')
                    ...
                    type_text(f'{pers[1]["name"]} также как и {pers[0]["name"]} ничего не замечает. Деревья как деревья.')
    elif choice_search == 4:
        type_text("Вы в страхе убираетесь с этой локации")
        return
    ...
    list_monsters_id = [19]
    fight.fight(list_characters_id, list_monsters_id)