...
def first_story():
    ...
    # opening scene
    a = '''Добро пожаловать в приключение! После успешных и не очень скитаний по миру вы прибыли в город 
    ...
    def steal_bread():
        while True:
            try:
                ...
                if choice == 1:
                    "тут должна быть функция для спасброска кости на ловкость"
                    return 1
                elif choice == 2:
                    "Тогда диалог просто продолжается дальше"
                    return 2
                    
                else:
                    print("Неверное значение, введи 1, 2 или 3")
                    
            except ValueError:
                        print("Ошибка: введи число!")

    next_step = int(steal_bread())
...
Бартон: «Ох, да ладно тебе, Томас.
    ...
    beginbing_dialog = '''Человек-воин: «Что значит "неладное"?»
...
от отца Гидеона и старые записи Чарльза Буна.)
            ...
            '''
    if next_step == 1:
        sort.type_text("""Человек-воин (наклоняясь к плуту, тихо):
...
в заброшенном городе “Жребий Иерусалима”. Он столкнулся с чем-то необъяснимым, неведомым… и просил помощи»
    """)
        sort.type_text(beginbing_dialog)
...
то, как этот дворф дал деру от циклопа. 
...
Для броска кубика напиши: 20 
    ...
    next_dialog = f'''Томас Грейсон (тихо, почти шёпотом):
